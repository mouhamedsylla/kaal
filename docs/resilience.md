...
| Signal détecté | Nœud activé | Ce que pilot fait |
|---|---|---|
| `prisma/schema.prisma` dans le projet | [3] migrations | `npx prisma migrate deploy` |
| `alembic.ini` dans le projet | [3] migrations | `alembic upgrade head` |
| `go-migrate` dans les dépendances | [3] migrations | `migrate up` |
| `flyway.conf` dans le projet | [3] migrations | `flyway migrate` |
| Service nginx avec bind-mount `.conf` | [6] post-hook | `nginx -s reload` |
| Services avec `depends_on` | [5] deploy | ordre topologique respecté |
| `healthcheck:` dans compose | [7] healthcheck | attend le statut `healthy` |

L'utilisateur ne configure rien de tout cela. pilot le découvre et agit en conséquence.
...
# pilot en déduit l'ordre :
# db → redis → api → worker
```

En cas de cycle détecté → erreur PILOT-CONFIG-001 avec le cycle identifié précisément.
...
Échec détecté au nœud [5] deploy
  ...
  │
  └─ rapport final :
       état AVANT  : image v1.2, schéma migration_007
       état APRÈS  : image v1.2 (restaurée), schéma migration_007 (revert OK)
       cause échec : container OOM au démarrage (512MB limit trop basse)
       fix suggéré : augmenter memory limit dans compose ou pilot.yaml
```

Si la compensation elle-même échoue → pilot le signale explicitement avec
l'état exact du système, sans tenter d'improviser davantage.
